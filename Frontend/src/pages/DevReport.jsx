...
export default function DevReport() {
  ...
  return <div className="dev-report">
    ...
    <div className="kpi-grid" style={{ marginBottom: 0 }}>
      ...
      <div className="glass-card kpi-card"><span className="kpi-label">Assistant mode</span><div className="kpi-value report-value-small">{assistant?.mode?.replaceAll('_', ' ') || 'Checking'}</div><span className="report-muted">{assistant?.model || 'Model not configured'}</span></div>
    ...
    <div className="dev-report-grid">
      ...
      <section className="glass-card"><div className="report-section-title"><Bot size={18} /><h4>Assistant & model configuration</h4></div><dl className="report-definition-list"><div><dt>Runtime mode</dt><dd>{assistant?.mode?.replaceAll('_', ' ') || 'Checking'}</dd></div><div><dt>Model</dt><dd>{assistant?.model || 'Not configured'}</dd></div><div><dt>Guardrail limit</dt><dd>{assistant?.guardrailLimit || 'Checking'}</dd></div><div><dt>Active sessions</dt><dd>{assistant?.activeSessions ?? 0}</dd></div><div><dt>API key</dt><dd>{assistant?.apiKeyConfigured ? 'Configured' : 'Not configured'}</dd></div></dl><p className="report-muted">The report only shows key presence; no secret value is returned.</p></section>
      <section className="glass-card"><div className="report-section-title"><Activity size={18} /><h4>API service checks</h4></div><div className="service-list">{(report?.services || []).map(service => <div className="service-row" key={service.name}><span>{service.name}<small>{service.path} · {service.detail}</small></span><StatusBadge status={service.status} label={`${service.httpStatus} · ${service.latencyMs}ms`} /></div>)}{!loading && !report?.services?.length && <span className="report-muted">No service results returned.</span>}</div><div className="report-section-title" style={{ marginTop: 18 }}><Boxes size={18} /><h4>Module diagnostics</h4></div><div className="service-list">{(report?.modules || []).map(module => <div className="service-row" key={module.name}><span>{module.name}<small>{module.detail}</small></span><StatusBadge status={module.status} /></div>)}</div></section>
